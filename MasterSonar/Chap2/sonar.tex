...
%% SJ: no transition. Mention both in the subsection introduction and then go on
...
%% SJ: while theoretically water surface or obstacles do have some refraction
%% Sj: involved, nobody would ever say that at the frequencies sonar work on
%%
%% SJ: the main source of refraction are the interfaces between water speed
%% SJ: zones
...
%% SJ: .... is that really needed ? It's feeling out of place
...
\begin{enumerate}
  ...
  \item \textbf{Side Scan} (a.k.a. towfish) - Can be either mounted on each side
      %% SJ: not sure about the beam shape, for the sidescan is usually more of
      %% SJ: profiler
...
%% SJ: actually, in the multibeam world, the only difference between profiling
%% SJ: imaging is the beam height. The data is presented in exactly the same way
%%
%% SJ: the thing is, profilers are more sensitive to errors in the measurement
%% SJ: and processing, so are harder to make
...
\begin{enumerate}
  ...
  %% SJ: "crispier image" is more an effect of generation than technique
