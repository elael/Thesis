...
\end{equation}
%
where $p_{\text{rms}}$ is the r.m.s.\abbrev{r.m.s.}{Root mean squared} value of the wave's pressure ({\small $\sqrt{\text{\tiny \(\overline{p^2}\)}}$}) and $p_0$ is
...
(TVG\abbrev{TVG}{Time Varying Gain}).
...
$72\%$ w.r.t.\abbrev{w.r.t.}{with respect to} the 1-way beam beamwidth. If the
...
(a.k.a.\abbrev{a.k.a.}{also known as} a grid), making it possible to detect a
full 3D direction.
